...
interface U { id: string; username: string; display_name: string | null }
interface B { id: string; name: string; code: string }
interface Item { brand_id: string; model_id: string; shipping_warehouse_id: string; quantity: number; unit: string; sale_price: number; amount: number; tax_rate: number; storage_fee_price: number; commission_fee_price: number; commission_fee: number }

const CONTRACT_TYPES = [
  { key: "SH", label: "送货", color: "border-sky-400 bg-sky-50 text-sky-700", dot: "bg-sky-500", active: "border-sky-500 bg-sky-100 text-sky-800 ring-2 ring-sky-200" },
  { key: "ZT", label: "自提", color: "border-amber-400 bg-amber-50 text-amber-700", dot: "bg-amber-500", active: "border-amber-500 bg-amber-100 text-amber-800 ring-2 ring-amber-200" },
  { key: "HH", label: "还货", color: "border-rose-400 bg-rose-50 text-rose-700", dot: "bg-rose-500", active: "border-rose-500 bg-rose-100 text-rose-800 ring-2 ring-rose-200" },
]

const EMPTY: Item = { brand_id: "", model_id: "", shipping_warehouse_id: "", quantity: 0, unit: "吨", sale_price: 0, amount: 0, tax_rate: 13, storage_fee_price: 0, commission_fee_price: 0, commission_fee: 0 }
...
export function SalesContractForm() {
  ...
  const { data: users } = useQuery({ queryKey: ["users-sales"], queryFn: async () => { const r = await apiGet<{ items: U[] }>("/auth/users?page_size=100"); return r.items } })
  ...
  const customers = allEnts?.filter(e => {
    const types: string[] = Array.isArray(e.enterprise_type) ? e.enterprise_type : [e.enterprise_type]
    return types.includes("trader") || types.includes("end_customer")
  }) ?? []

  useEffect(() => {
    if (!id) return
    apiGet<{ commission_platform_id: string | null; company_id: string; contract_no: string; contract_type: string; sales_person_id: string | null; customer_enterprise_id: string; contract_date: string; contract_start_date: string; contract_end_date: string; attachment_path: string | null; remark: string | null; items: Item[] }>(`/sales-contracts/${id}`).then(d => {
      setCommissionPlatformId(d.commission_platform_id ?? ""); setCompanyId(d.company_id ?? ""); setContractType(d.contract_type ?? "SH"); setSalesPersonId(d.sales_person_id ?? "")
      setSid(d.customer_enterprise_id); setDate(d.contract_date)
  ...
  const upd = (i: number, f: keyof Item, v: string | number) => {
    setItems(p => { const n = [...p]; const it = { ...n[i], [f]: v }; if (f === "quantity" || f === "sale_price") it.amount = Number((it.quantity * it.sale_price).toFixed(2)); n[i] = it; return n })
  }
  ...
  const mut = useMutation({
    mutationFn: async () => {
      const body = { company_id: companyId, contract_type: contractType, sales_person_id: salesPersonId || null, customer_enterprise_id: sid, commission_platform_id: commissionPlatformId || null, contract_date: date, contract_start_date: startDate, contract_end_date: endDate, attachment_path: attach.length > 0 ? attach : null, remark: remark || null, items }
      console.log("SALES BODY:", JSON.stringify(body))
      return isEdit ? apiPut(`/sales-contracts/${id}`, body) : apiPost("/sales-contracts", body)
    },
    onSuccess: () => nav("/sales-contracts"),
    onError: (err: any) => sysToast("提交失败: " + (err.message || "未知错误")),
  })

  const ok = companyId && sid && startDate && endDate && !items.some(i => !i.brand_id || i.quantity <= 0)
  ...
  return (
    <div className="page-enter p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">
      <div className="flex items-center gap-2 text-slate-400 text-sm mb-2">
        ...
        <a onClick={() => nav("/sales-contracts")} className="hover:text-slate-600 cursor-pointer">销售合同</a><span>/</span>
        <span className="text-slate-700 font-medium">{isEdit ? "编辑" : "新增"}</span>
      </div>
      <h1 className="text-xl font-bold text-slate-900 mb-5">{isEdit ? "编辑销售合同" : "新增销售合同"}</h1>
      ...
      <div className="bg-white rounded-xl border border-slate-200/60 shadow-sm p-5 mb-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-3">
          ...
          <div className="flex items-center gap-3">
            <span className="text-xs text-slate-400 w-16 shrink-0">客户 {!sid && <span className="text-rose-400">*</span>}</span>
            <SearchableSelect value={sid} onChange={v => setSid(v)} options={customers} className={INP + " !min-w-[200px]"} />
            <AddEnterpriseModal onAdded={(newId) => setSid(newId)} defaultTypes={["end_customer"]} title="新增客户" />
          </div>
          <div className="flex items-center gap-3">
            <span className="text-xs text-slate-400 w-16 shrink-0">销售人员</span>
            <SearchableSelect value={salesPersonId} onChange={setSalesPersonId} options={(users ?? []).map(u => ({ id: u.id, name: u.display_name || u.username }))} className={INP + " !min-w-[200px]"} />
          </div>

          <div className="flex items-center gap-3">
            <span className="text-xs text-slate-400 w-16 shrink-0">合同类型 {!contractType && <span className="text-rose-400">*</span>}</span>
            <div className="flex gap-2 flex-1">
              {CONTRACT_TYPES.map(ct => (
                <button key={ct.key} type="button" onClick={() => setContractType(ct.key)}
                  className={`px-3 py-1 rounded-lg border text-xs font-medium transition-all ${contractType === ct.key ? ct.active : ct.color + " hover:opacity-80"}`}>
                  {ct.label}
                </button>
              ))}
            </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-y border-slate-100">
                ...
                <th className="px-2.5 py-2 text-xs font-semibold text-slate-400 text-right w-20">销售单价 <span className="text-rose-400">*</span></th>
              ...
              {items.map((it, i) => {
                ...
                return (
                  <tr key={i} className="border-b border-slate-50 hover:bg-indigo-50/20 transition-colors group">
                    ...
                    <td className="px-2.5 py-1.5 w-24"><input type="number" step="0.01" value={it.sale_price || ""} onChange={e => upd(i, "sale_price", parseFloat(e.target.value) || 0)} className={INP + " text-right"} /></td>
      ...
      <div className="sticky bottom-4 bg-white/95 backdrop-blur-xl rounded-xl border border-slate-200/60 shadow-lg shadow-slate-200/30 p-5 flex items-center">
        <div className="flex items-center gap-8">
          ...
          <div><span className="text-xs text-slate-400">已回</span><p className="text-sm text-slate-700 tabular-nums">{paid.toLocaleString()}</p></div>
          <div><span className="text-xs text-slate-400">已提</span><p className="text-sm text-slate-700 tabular-nums">{shipped.toLocaleString()}</p></div>
          <div className="w-28">
            <div className="flex justify-between text-xs mb-0.5"><span className="text-slate-400">回款比例</span><span className="text-slate-500">{tAmt > 0 ? Math.round(paid / tAmt * 100) : 0}%</span></div>
          ...
          <div className="w-28">
            <div className="flex justify-between text-xs mb-0.5"><span className="text-slate-400">提货比例</span><span className="text-slate-500">{tQty > 0 ? Math.round(shipped / tQty * 100) : 0}%</span></div>
        ...
        <button onClick={() => nav("/sales-contracts")} className="px-4 py-2 border border-slate-200 rounded-lg text-sm font-medium text-slate-600 hover:bg-slate-50 transition-colors shrink-0">取消</button>
