...
interface Item { id: string; line_no: number; brand_id: string; model_id: string; shipping_warehouse_id: string; quantity: number; unit: string; sale_price: number; amount: number; tax_rate: number; storage_fee_price: number; commission_fee_price: number; commission_fee: number }
interface Detail { id: string; contract_no: string; customer_enterprise_id: string; contract_date: string; contract_start_date: string; contract_end_date: string; attachment_path: string | null; status: string; total_quantity: number; total_amount: number; pickup_progress: number; collection_progress: number; remark: string | null; created_at: string; items: Item[] }
...
export function SalesContractDetail() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ["sc-detail", id], queryFn: () => apiGet<Detail>(`/sales-contracts/${id}`), enabled: !!id })
  ...
  const del = useMutation({ mutationFn: () => apiDelete(`/sales-contracts/${id!}`), onSuccess: () => { qc.invalidateQueries({ queryKey: ["sc"] }); nav("/sales-contracts") } })
    ...
    <div className="page-enter p-8 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-2" ref={menuRef}>
          <Link to={`/sales-contracts/${id}/edit`} className="px-4 py-2.5 border border-slate-200 rounded-xl text-sm font-medium text-slate-600 hover:bg-slate-50 transition-colors">编辑</Link>
      ...
      <div className="grid grid-cols-3 gap-6 mb-8">
        <div className="col-span-2 bg-white rounded-xl border border-slate-200/60 shadow-sm p-5">
          ...
          <div className="grid grid-cols-2 gap-x-8 gap-y-3">
            {[["合同编号", data.contract_no], ["客户", en(data.customer_enterprise_id)], ["签订日期", data.contract_date], ["合同期限", `${data.contract_start_date} ~ ${data.contract_end_date}`], ["备注", data.remark || ""], ["创建日期", data.created_at?.slice(0, 10)]].map(([l, v]) => (
        ...
        <div className="bg-white rounded-xl border border-slate-200/60 shadow-sm p-5">
          ...
          <div className="space-y-5">
            {[["提货进度", data.pickup_progress, "from-blue-500 to-indigo-500"], ["回款进度", data.collection_progress, "from-emerald-500 to-teal-500"]].map(([l, pct, g]) => (
      ...
      <div className="bg-white rounded-xl border border-slate-200/60 shadow-sm p-5 mb-8">
        ...
        <table className="w-full text-sm">
          <thead><tr className="border-y border-slate-100">{["#", "品牌", "型号", "提货仓库", "数量(吨)", "销售单价", "金额", "税率", "库费单价", "撮合费单价", "撮合费"].map(h => (<th key={h} className={`px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider ${["数量(吨)","销售单价","金额","税率","库费单价","撮合费单价","撮合费"].includes(h)?"text-right":"text-left"}`}>{h}</th>))}</tr></thead>
          <tbody>
            {data.items.map(it => (
              <tr key={it.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                ...
                <td className="px-4 py-3.5 text-right text-slate-700 tabular-nums">¥{it.sale_price.toLocaleString()}</td>
      ...
      <AuditTimeline entityType="sales_contract" entityId={data.id} />
    </div>
  )
}
