...
interface Payment { id: string; payment_no: string; bp_id: string; type: string; amount: number; payment_date: string; method: string; status: string; remark: string | null }
...
export function APLedger() {
  ...
  const { data: payments } = useQuery({
    queryKey: ["ap-payments", page, pageSize],
    queryFn: async () => apiGet<{ items: Payment[]; total: number; pages: number }>(`/finance/ap/payments?page=${page}&page_size=${pageSize}`),
  })
  ...
  const bpOptions = useMemo(() => {
    ...
    return (payments?.items ?? []).filter(p => { if (seen.has(p.bp_id)) return false; seen.add(p.bp_id); return true }).map(p => ({ id: p.bp_id, name: (enterprises ?? {})[p.bp_id] ?? p.bp_id.slice(0, 6) }))
  }, [payments, enterprises])

  /** 确认付款 */
  const confirmMut = useMutation({
    mutationFn: (id: string) => apiPost(`/finance/ap/payments/${id}/confirm`),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["ap-payments"] }); setDetail(null) },
  })

  /** 作废付款 */
  const voidMut = useMutation({
    mutationFn: (id: string) => apiDelete(`/finance/ap/payments/${id}`),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["ap-payments"] }); setDetail(null) },
  })

  const items = payments?.items ?? []; const total = payments?.total ?? 0; const pages = payments?.pages ?? 0
  const filtered = useMemo(() => {
    let arr = items
    if (fBpId) arr = arr.filter(p => p.bp_id === fBpId)
  ...
  return (
    <div className="page-enter p-4 sm:p-6 lg:p-8">
      <h1 className="text-xl font-bold text-slate-900 mb-4">应付账款</h1>
      ...
      <div className="bg-white rounded-xl border border-slate-200/60 shadow-sm overflow-x-auto -mx-4 sm:mx-0">
        <table className="w-full text-sm">
          <thead><tr className="border-b border-slate-100 bg-slate-50/50">
            {["编号","供方","类型","金额","日期","方式","状态","备注"].map(h => <th key={h} className="px-3 py-2.5 text-xs font-semibold text-slate-400 text-left">{h}</th>)}
          </tr></thead>
          <tbody>
            {items.map(p => (
              <tr key={p.id} className="border-b border-slate-50 hover:bg-indigo-50/30 cursor-pointer" onClick={() => setDetail(p)}>
                <td className="px-3 py-2.5 text-xs font-semibold text-indigo-600">{p.payment_no}</td>
                <td className="px-3 py-2.5 text-xs text-slate-700">{(enterprises ?? {})[p.bp_id] ?? p.bp_id.slice(0, 8)}</td>
                <td className="px-3 py-2.5 text-xs text-slate-600">{TYPE_LABELS[p.type] ?? p.type}</td>
                <td className="px-3 py-2.5 text-xs font-semibold text-slate-800">¥{fmtMoney(p.amount)}</td>
                <td className="px-3 py-2.5 text-xs text-slate-600">{p.payment_date}</td>
                <td className="px-3 py-2.5 text-xs text-slate-500">{p.method === "transfer" ? "转账" : p.method === "cash" ? "现金" : "承兑"}</td>
                <td className="px-3 py-2.5 text-xs"><span className={`px-1.5 py-0.5 rounded-full text-xs ${p.status === "confirmed" ? "bg-emerald-50 text-emerald-600" : p.status === "voided" ? "bg-red-50 text-red-400" : "bg-amber-50 text-amber-600"}`}>{STATUS_LABELS[p.status]}</span></td>
                <td className="px-3 py-2.5 text-xs text-slate-400 max-w-[120px] truncate">{p.remark || ""}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative w-[480px] bg-white shadow-2xl h-full overflow-y-auto">
            <div className="sticky top-0 bg-white border-b px-6 py-4 flex items-center justify-between z-10">
              <div><h2 className="text-lg font-bold">{detail.payment_no}</h2><span className="text-xs text-slate-400">{STATUS_LABELS[detail.status]}</span></div>
              <div className="flex items-center gap-2">
                {detail.status === "pending" && (
                  <>
                    ...
                    <button
                      onClick={async () => { if (sysConfirm("确定作废此付款？")) voidMut.mutate(detail.id) }}
            ...
            <div className="p-6 space-y-3 text-sm">
              <div className="grid grid-cols-2 gap-2">
                <div><span className="text-slate-400 text-xs">供方</span><div className="text-slate-800">{(enterprises ?? {})[detail.bp_id] ?? "—"}</div></div>
                ...
                <div><span className="text-slate-400 text-xs">日期</span><div className="text-slate-800">{detail.payment_date}</div></div>
              ...
              <AuditTimeline entityType="ap_payment" entityId={detail.id} />
            </div>
          </div>
        </div>
      )}
