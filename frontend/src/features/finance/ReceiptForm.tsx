...
export function ReceiptForm() {
  ...
  return (
    <div className="page-enter p-6 max-w-xl mx-auto">
      <h1 className="text-xl font-bold text-slate-900 mb-6">新增收款</h1>
      <div className="bg-white rounded-xl border border-slate-200/60 shadow-sm p-6 space-y-4">
        ...
        <div>
          <label className="block text-xs text-slate-500 mb-1.5">客户 {!bpId && <span className="text-rose-400">*</span>}</label>
          <SearchableSelect value={bpId} onChange={setBpId} options={enterprises ?? []} className={INP + " !min-w-[200px]"} />
        </div>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-xs text-slate-500 mb-1.5">收款类型 {!type && <span className="text-rose-400">*</span>}</label>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-xs text-slate-500 mb-1.5">收款日期 {!date && <span className="text-rose-400">*</span>}</label>
            <input type="date" value={date} onChange={e => setDate(e.target.value)} className={INP} />
          </div>
          <div>
            <label className="block text-xs text-slate-500 mb-1.5">收款方式</label>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div><label className="block text-xs text-slate-500 mb-1.5">付款银行</label><input value={bankName} onChange={e => setBankName(e.target.value)} className={INP} /></div>
          <div><label className="block text-xs text-slate-500 mb-1.5">付款账号</label><input value={bankAccount} onChange={e => setBankAccount(e.target.value)} className={INP} /></div>
        </div>
