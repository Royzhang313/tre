...
interface C { id: string; contract_no: string; supplier_enterprise_id: string; contract_date: string; status: string; total_quantity: number; total_amount: number; delivery_progress: number; payment_progress: number; contract_start_date: string; contract_end_date: string; attachment_path: { path: string; filename: string }[] | null; items: ItemL[] | null; tags: Tag[] | null; remark: string | null }
...
function getStatus(c: C) {
  const dp = c.delivery_progress ?? 0; const pp = c.payment_progress ?? 0
  ...
  if (dp > pp) return { l: "待付款", c: "bg-rose-50 text-rose-600 border-rose-200", k: "pending_payment" }
  if (pp > dp) return { l: "待发货", c: "bg-orange-50 text-orange-600 border-orange-200", k: "pending_delivery" }
...
function TagCell({ contractId, tags: initialTags }: { contractId: string; tags: Tag[] | null }) {
  ...
  const save = async (newTags: Tag[]) => {
    setTags(newTags)
    try { await apiPatch(`/purchase-contracts/${contractId}/tags`, { tags: newTags }) } catch { /* 忽略 */ }
  }
...
export function PurchaseContractList() {
  ...
  const statusOptions = [{v:"new",l:"新"},{v:"pending",l:"待执行"},{v:"pending_payment",l:"待付款"},{v:"pending_delivery",l:"待发货"},{v:"done",l:"完成"},{v:"cancelled",l:"已作废"}]

  const { data, isLoading } = useQuery({
    queryKey: ["pc", page],
    queryFn: () => apiGet<{ items: C[]; total: number; pages: number }>(`/purchase-contracts?page=${page}&page_size=100`),
  })
  ...
  const stats = useMemo(() => {
    const s = { new: 0, pending_payment: 0, pending_delivery: 0, pending: 0, done: 0, cancelled: 0, total: filtered.length }
  ...
  return (
    <div className="page-enter p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-3">
        <h1 className="text-xl font-bold text-slate-900">采购合同</h1>
        <Link to="/purchase-contracts/create" className="inline-flex items-center gap-1.5 px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-all text-sm font-medium shadow-sm">+ 新增</Link>
      </div>
      ...
      <div className="flex gap-3 mb-3 text-xs">
        {[["全部", stats.total, "bg-slate-100 text-slate-600"], ["新", stats.new, "bg-emerald-50 text-emerald-700"], ["待执行", stats.pending, "bg-blue-50 text-blue-700"], ["待付款", stats.pending_payment, "bg-rose-50 text-rose-600"], ["待发货", stats.pending_delivery, "bg-orange-50 text-orange-600"], ["完成", stats.done, "text-slate-400"]].map(([l, v, c]) => (
        ...
        {isLoading ? <div className="p-16 text-center text-slate-300">加载中...</div> : filtered.length === 0 ? (
          ...
          <table className="w-full text-sm">
            <thead><tr className="border-b border-slate-100 bg-slate-50/50">
              {["标签","状态","签订日期","合同编号","供方","明细","数量","金额","已发","可发","已付","待付","附件","备注"].map(h => (
                <th key={h} className={`px-3 py-2.5 text-xs font-semibold text-slate-400 ${["标签","状态","附件"].includes(h)?"text-center":["数量","金额","已发","可发","已付","待付"].includes(h)?"text-right":"text-left"}`}>{h}</th>
              ))}
            </tr></thead>
            <tbody>
              {filtered.map(c => (
                <tr key={c.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                  ...
                  {/* 供方 */}
                  <td className="px-3 py-2.5 text-slate-700 font-medium text-xs">{nm(c.supplier_enterprise_id)}</td>
                  {/* 明细 —— 多行换行显示 */}
                  ...
                  <td className="px-2 py-2.5 text-right">
                    <span className="text-xs text-slate-500 font-medium tabular-nums">{Math.round(c.total_quantity * (c.delivery_progress ?? 0) / 100).toLocaleString()}</span>
                    <span className="text-[10px] text-slate-400 ml-0.5">吨</span>
                  </td>
                  {/* 可发 */}
                  <td className="px-2 py-2.5 text-right">
                    <span className="text-xs text-emerald-600 font-medium tabular-nums">{Math.round(c.total_quantity * (100 - (c.delivery_progress ?? 0)) / 100).toLocaleString()}</span>
                    <span className="text-[10px] text-slate-400 ml-0.5">吨</span>
                  </td>
                  {/* 已付 */}
                  <td className="px-2 py-2.5 text-right">
                    <span className="text-xs text-slate-500 font-medium tabular-nums">¥{Math.round(c.total_amount * (c.payment_progress ?? 0) / 100).toLocaleString()}</span>
                  </td>
                  {/* 待付 */}
                  <td className="px-2 py-2.5 text-right">
                    <span className="text-xs text-rose-400 font-medium tabular-nums">¥{Math.round(c.total_amount * (100 - (c.payment_progress ?? 0)) / 100).toLocaleString()}</span>
                  </td>
                  {/* 附件 */}
...
function PreviewModal({ id, idMap, ents, companies, onClose }: { id: string; idMap: any; ents: any; companies: any; onClose: () => void }) {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["pc-preview", id],
    queryFn: () => apiGet<any>(`/purchase-contracts/${id}`),
    enabled: !!id,
  })

  const cancelMut = useMutation({
    mutationFn: () => apiDelete(`/purchase-contracts/${id}`),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["pc"] }); onClose() }
  })
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[92vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="sticky top-0 bg-white/95 backdrop-blur-xl border-b border-slate-100 px-8 py-5 flex items-center justify-between rounded-t-2xl z-10">
          ...
          <div className="flex items-center gap-2" ref={menuRef}>
            <Link to={`/purchase-contracts/${id}/edit`} className="px-4 py-2.5 border border-slate-200 rounded-xl text-sm font-medium text-slate-600 hover:bg-slate-50 transition-colors" onClick={onClose}>编辑</Link>
            <div className="relative">
          ...
          <div className="grid grid-cols-3 gap-6">
            <div className="col-span-2 bg-white rounded-xl border border-slate-200/60 p-5">
              ...
              <div className="grid grid-cols-2 gap-x-8 gap-y-3">
                ...
                <div className="flex items-center gap-3"><span className="text-xs text-slate-400 w-20 shrink-0">供方</span><span className="text-sm font-medium text-slate-800">{en(c.supplier_enterprise_id)}</span></div>
            ...
            <div className="bg-white rounded-xl border border-slate-200/60 p-5">
              ...
              <div className="space-y-5">
                <div>
                  <div className="flex justify-between text-xs mb-1.5"><span className="text-slate-500">发货进度</span><span className="font-semibold text-slate-700">{c.delivery_progress??0}%</span></div>
                  <div className="h-2.5 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-emerald-500 rounded-full" style={{width:`${c.delivery_progress??0}%`}}/></div>
                  <div className="flex justify-between text-xs mt-1"><span className="text-slate-400">已发</span><span className="font-medium text-slate-600 tabular-nums">{Math.round(c.total_quantity * (c.delivery_progress ?? 0) / 100).toLocaleString()} 吨</span></div>
                </div>
                <div>
                  <div className="flex justify-between text-xs mb-1.5"><span className="text-slate-500">付款进度</span><span className="font-semibold text-slate-700">{c.payment_progress??0}%</span></div>
                  <div className="h-2.5 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-blue-500 rounded-full" style={{width:`${c.payment_progress??0}%`}}/></div>
                  <div className="flex justify-between text-xs mt-1"><span className="text-slate-400">已付</span><span className="font-medium text-slate-600 tabular-nums">¥{Math.round(c.total_amount * (c.payment_progress ?? 0) / 100).toLocaleString()}</span></div>
                </div>
              </div>
            </div>
          </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>{(c.items??[]).map((it: any, i: number) => (
                <tr key={i} className="border-b border-slate-50">
                  ...
                  <td className="px-3 py-2.5 text-right text-slate-700">¥{it.purchase_price?.toLocaleString()}</td>
