...
from app.modules.purchase_contract.service import PurchaseContractService
from app.shared.base_schema import APIResponse, PageResponse
from app.shared.orm_utils import orm_to_dict as _to_dict

router = APIRouter(prefix="/purchase-contracts", tags=["采购合同"])




async def _make_svc() -> AsyncGenerator[PurchaseContractService, None]:
    async with async_session_factory() as session:
        yield PurchaseContractService(repo=PurchaseContractRepository(session), item_repo=PurchaseContractItemRepository(session))
...
@router.post("")
async def create(body: PurchaseContractCreate, svc=Depends(_make_svc)):
    try:
...
async def list_contracts(page: int = Query(default=1, ge=1), page_size: int = Query(default=20, ge=1, le=100)):
    async with async_session_factory() as session:
        repo = PurchaseContractRepository(session)
...
async def update(contract_id: UUID, body: PurchaseContractUpdate, svc=Depends(_make_svc)):
